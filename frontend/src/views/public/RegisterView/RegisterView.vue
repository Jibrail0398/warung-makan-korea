<template>
  <AuthLayout title="Daftar Akun">
    <form @submit.prevent="handleRegister" class="auth-form" novalidate>
      ...
      <AuthButton
        :text="isLoading ? 'Memproses...' : 'Kirim OTP'"
        :loading="isLoading"
        type="submit"
        variant="primary"
      />
      ...
      <router-link to="/" class="guest-btn-link">
        <AuthButton text="Lanjutkan sebagai Guest" type="button" variant="secondary" />
      </router-link>

      <div class="auth-footer-nav">
        <span>Sudah memiliki akun?</span>
        <router-link to="/login" class="auth-switch-link">Masuk</router-link>
      </div>
    </form>
  </AuthLayout>
</template>
