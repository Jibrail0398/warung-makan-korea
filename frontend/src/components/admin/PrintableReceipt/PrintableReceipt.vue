    ...
    <div v-if="isOpen" class="print-modal-backdrop" @click.self="$emit('close')">
      <div class="print-dialog" role="dialog" aria-modal="true" aria-labelledby="printTitle">
        ...
        <div id="receipt-print-area" class="receipt-preview">
          ...
          <div class="receipt-divider"></div>
          <div class="receipt-items-heading"><strong>Item pesanan</strong><strong>Jumlah</strong></div>
          <table class="receipt-items"><tbody><tr v-for="item in order?.items" :key="item.id"><td class="col-item"><span class="item-title">{{ item.product?.name || 'Produk' }} x {{ item.quantity }}</span></td><td class="col-total">{{ formatPrice(item.sub_total || item.price * item.quantity) }}</td></tr></tbody></table>
          <div class="receipt-divider receipt-divider-dark"></div>
          <div class="receipt-totals"><div class="total-row grand-total"><strong>Total</strong><strong class="total-amount">{{ formatPrice(order?.total_price) }}</strong></div></div>
          <div class="receipt-footer"><p class="footer-msg">Terima Kasih Atas Kunjungan Anda!</p><p class="footer-korean">감사합니다 / Selamat Menikmati</p></div>
        </div>
      </div>
    </div>
  </Transition>
...
export default {
  ...
  setup(props) {
    ...
    // Download struk kertas A6 — desain sama dengan sisi member
